  ...
  var exdateParam = function (name) {
      return function (val, params, curr) {
          ...
          val.split(',').forEach(v => {
              ...
              // console.log(`v = ${v}`)
              dateParam(name)(v, params, exdate);
              // console.log(`curr = ${util.inspect(curr)}`)
              curr[name] = curr[name] || [];
              // console.log(`exdate[name] = ${exdate[name]}`)
    ...
    parseLines : function(lines, limit, ctx, stack, lastIndex, cb){
      ...
      for (var ii = lines.length; i<ii; i++){
        ...
        try {
            ...
            console.log(`Error in parseLines with ${name}, ${value}, ${params}`)
        }
